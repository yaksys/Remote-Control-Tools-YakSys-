using System;


public class StringFactory
{
    public static string GetString(int int_StringNumber, int int_Language)
    {
        switch (int_StringNumber)
        {
            case 1:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "JurikSoft Server Service Installer";


                        case 1:
                            return "JurikSoft Server Service Installer";

                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 2:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Internet: http://www.juriksoft.net\n\nAuthor: jurik@juriksoft.net\n\nTechnical support: support@juriksoft.net\n\nInformation: info@juriksoft.net\n\nSales questios: sales@juriksoft.net\n\nICQ: 55-23-99-11";


                        case 1:
                            return "Интернет сайт: http://www.juriksoft.net\n\nАвтор: jurik@juriksoft.net\n\nТехническая поддержка: support@juriksoft.net\n\nИнформация: info@juriksoft.net\n\nПо вопросам продаж: sales@juriksoft.net\n\nICQ: 55-23-99-11";

                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 3:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "About";

                        case 1:
                            return "О Программе";
                    }
                }
                break;
            /////////////////////////////////////////////////////////	

            case 4:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Host Address:";


                        case 1:
                            return "Адрес Хоста:";

                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 5:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Host Type:";


                        case 1:
                            return "Тип Хоста:";

                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 6:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Service Type:";


                        case 1:
                            return "Тип Сервиса:";

                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 7:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Desktop Interact:";


                        case 1:
                            return "Интерактивность:";

                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 8:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Service Status:";


                        case 1:
                            return "Статус Сервиса:";

                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 9:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Service Name:";

                        case 1:
                            return "Имя Сервиса:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 10:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Service State:";

                        case 1:
                            return "Состояние Сервиса:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 11:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Process ID:";

                        case 1:
                            return "ID процесса:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 12:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Service Start Mode:";

                        case 1:
                            return "Режим запуска сервиса:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 13:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Automatiс";

                        case 1:
                            return "Автоматически";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 14:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Manual";

                        case 1:
                            return "В ручную";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 15:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Disabled";

                        case 1:
                            return "Отключен";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 16:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Get Information";

                        case 1:
                            return "Получить инфрмацию";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 17:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Network Address:";

                        case 1:
                            return "Сетевой адрес:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 18:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Login:";

                        case 1:
                            return "Логин:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 19:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Domain:";

                        case 1:
                            return "Домен:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 20:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Password:";

                        case 1:
                            return "Пароль:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 21:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Microsoft Windows Network";

                        case 1:
                            return "Сеть Microsoft Windows";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 22:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Active Directory Network";

                        case 1:
                            return "Сеть Active Directory";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 23:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Install Server";

                        case 1:
                            return "Установить сервер";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 24:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Uninstall Server";

                        case 1:
                            return "Удалить сервер";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 25:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Networks and Computers";

                        case 1:
                            return "Сети и Компьютеры";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 26:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "JurikSoft Server Service Control and Information";

                        case 1:
                            return "Информация и управление JurikSoft Server Service";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 27:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Cancel";

                        case 1:
                            return "Отмена";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 28:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Task Progrress";

                        case 1:
                            return "Ход выполнения задания";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 29:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Task Activity Monitoring";

                        case 1:
                            return "Контроль выполнения задания";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 30:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Access Denied.";

                        case 1:
                            return "Доступ запрещен.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 31:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The type of device and the type of network resource do not match.";

                        case 1:
                            return "Тип устойства и тип сетевого ресурса не совпадают.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////
                
            case 32:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The specified Local Name is invalid.";

                        case 1:
                            return "Указанное Локальное Имя не верно.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 33:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The Remote resource is not acceptable to any network resource provider, either because the resource name is invalid, or because the named resource cannot be located.";

                        case 1:
                            return "Удаленный ресурс не доступен для любой сети провадера ресурсов, возможно потому что имя ресурса не верно или потому что ресурс не был обнаружен.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 34:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The user account is in an incorrect format.";

                        case 1:
                            return "Учетная запись пользователя имеет некорректный формат.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 35:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The Provider value does not match any provider.";

                        case 1:
                            return "Значение Провайдера не соответствует никаком из провайдеров.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 36:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The router or provider is busy, possibly initializing. Please retry later.";

                        case 1:
                            return "Роутер или провайдер заняты, возможно по причине выполнения процесса инициализации. Попробуйие повторить поытку через некоторое время.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 37:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The attempt to make the connection was cancelled by the user through a dialog box from one of the network resource providers, or by a called resource.";

                        case 1:
                            return "Попытка установить соединение была отменена пользователем через диалоговое окно одного из сетевых провайдеров ресурсов или вызываемым ресурсом.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 38:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The system is unable to open the user profile to process persistent connections.";

                        case 1:
                            return "Система не смогла открыть профиль пользователя для обработки постоянных соединений.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 39:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "An entry for the device specified by LocalName is already in the user profile.";

                        case 1:
                            return "Вход для устройства определенного LocalName уже находится в пользовательском профиле.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 40:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "A network-specific error occurred.";

                        case 1:
                            return "Произошла определенная сетевая ошибка.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 41:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The specified password is invalid.";

                        case 1:
                            return "Указанный пароль не верен.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 42:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The operation cannot be performed because a network component is not started or because a specified name cannot be used.";

                        case 1:
                            return "Операция не может быть завершена в по причине невозможности запуска сетевого компонента или в связи с тем что указанное имя не может использоваться.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 43:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The network is unavailable.";

                        case 1:
                            return "Сеть недоступна.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 44:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Authorization failure.";

                        case 1:
                            return "Отказано в доступе.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 45:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Multiple connections to a server or shared resource by the same user, using more than one user name, are not allowed. Disconnect all previous connections to the server or shared resource and try again.";

                        case 1:
                            return "Множественный подключения к серверу или общим ресурсам одного и того же пользователя, используя более чем одно имя пользователя, недопустимо. Отключите все предыдущее соединения к серверу или общим ресурсам и попробуйте снова.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 46:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "OK, Object already exist";

                        case 1:
                            return "ОК, Объект уже существует.";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 47:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Installation process was aborted";

                        case 1:
                            return "Процесс установки был прерван";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 48:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Installation process was cancelled";

                        case 1:
                            return "Процесс установки был отменен";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 49:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Uninstallation process was cancelled";

                        case 1:
                            return "Процесс удаления приложения был отменен";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 50:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "User Auth";

                        case 1:
                            return "Авторизация Пользователя";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 51:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Creating JurikSoft Server directory";

                        case 1:
                            return "Создание директории JurikSoft Server";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 52:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Self-extracted file сopying";

                        case 1:
                            return "Копирование самораспаковывающегося архива";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 53:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Getting JurikSoft Server directory path";

                        case 1:
                            return "Получени пути к директории JurikSoft Server";
                    }
                }
                break;

            /////////////////////////////////////////////////////////


            case 54:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Exctracting JurikSoft Server binaries";

                        case 1:
                            return "Извлечение исполняемых файлов JurikSoft Server";
                    }
                }
                break;

            /////////////////////////////////////////////////////////


            case 55:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Creating and registering JurikSoft Server Windows Service";

                        case 1:
                            return "Создание и регистрация Windows службы JurikSoft Server";
                    }
                }
                break;

            /////////////////////////////////////////////////////////


            case 56:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Starting JurikSoft Server Windows Service";

                        case 1:
                            return "Запуск Windows службы JurikSoft Server";
                    }
                }
                break;

            /////////////////////////////////////////////////////////


            case 57:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Starting JurikSoft Server Windows Service";

                        case 1:
                            return "Запуск Windows службы JurikSoft Server";
                    }
                }
                break;

            /////////////////////////////////////////////////////////


            case 58:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Installation Complete";

                        case 1:
                            return "Установка завершена";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

                
            case 59:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Stop JurikSoft Server Windows Service";

                        case 1:
                            return "Остановка Windows службы JurikSoft Server";
                    }
                }
                break;

            /////////////////////////////////////////////////////////


            case 60:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Delete JurikSoft Server Service";

                        case 1:
                            return "Удаление JurikSoft Server Windows Service";
                    }
                }
                break;

            /////////////////////////////////////////////////////////


            case 61:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Delete JurikSoft Server directory";

                        case 1:
                            return "Удаление директории JurikSoft Server";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

                
            case 62:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Uninstallation Complete";

                        case 1:
                            return "Удаление приложения завершено";
                    }
                }
                break;

            /////////////////////////////////////////////////////////


            case 63:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Domain";

                        case 1:
                            return "Домен";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

                
            case 64:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Computer";

                        case 1:
                            return "Компьютер";
                    }
                }
                break;

            /////////////////////////////////////////////////////////

            case 65:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Working activity monitoring process was cancelled";

                        case 1:
                            return "Процесс мониторинга активности работы был отменен";
                    }
                }
                break;
            /////////////////////////////////////////////////////////


            case 66:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "The device is already connected to a network resource.";

                        case 1:
                            return "Устройство уже подключено к сетевому ресурсу";
                    }
                }
                break;
            /////////////////////////////////////////////////////////
                

            case 67:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Remote System";

                        case 1:
                            return "Удаленная система";
                    }
                }
                break;
            /////////////////////////////////////////////////////////
                

            case 68:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Local System";

                        case 1:
                            return "Локальная система";
                    }
                }
                break;
            /////////////////////////////////////////////////////////


            case 69:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Install Date:";

                        case 1:
                            return "Дата установки:";
                    }
                }
                break;
            /////////////////////////////////////////////////////////

            case 70:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Operation";

                        case 1:
                            return "Операция";
                    }
                }
                break;
            /////////////////////////////////////////////////////////


            case 71:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Result";

                        case 1:
                            return "Результат";
                    }
                }
                break;
            /////////////////////////////////////////////////////////


            case 72:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Working Progress";

                        case 1:
                            return "Прогресс выполнения";
                    }
                }
                break;
            /////////////////////////////////////////////////////////
                
            case 73:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Working Activity";

                        case 1:
                            return "Мониторинг работы";
                    }
                }
                break;
            /////////////////////////////////////////////////////////



            case 74:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "File";

                        case 1:
                            return "Файл";
                    }
                }
                break;



            case 75:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Exit";

                        case 1:
                            return "Выход";
                    }
                }
                break;



            case 76:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Language";

                        case 1:
                            return "Язык";
                    }
                }
                break;



            case 77:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Russian";

                        case 1:
                            return "Русский";
                    }
                }
                break;



            case 78:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Help";

                        case 1:
                            return "Помощь";
                    }
                }
                break;


            case 79:
                {
                    switch (int_Language)
                    {
                        case 0:
                            return "Are you sure want to exit?";

                        case 1:
                            return "Вы действительно хотите выйти?";
                    }
                }
                break;
            //////	


            default:
                {
                    return "";
                }
        }

        return "";
    }
}
